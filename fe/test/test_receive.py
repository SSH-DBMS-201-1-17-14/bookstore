...
class Testreceive:
    ...
    def test_error_non_user(self):
        ...
        code = self.buyer.receive(self.buyer_id + "_x", self.order_id)
        assert code == 511

    def test_error_non_order(self):
        code = self.seller.deliver(self.seller_id, self.order_id)
        assert code == 200

        code = self.buyer.receive(self.buyer_id, self.order_id + "_x")
        assert code == 522
    ...
    def test_error_buyer_order_belong(self):
        ...
        code = self.buyer.receive(self.another_buyer_id, self.order_id)
        assert code == 541

    def test_error_diliver_flag(self):
