...
class TestAddStockLevel:
    ...
    def pre_run_initialization(self):
        ...
        self.another_seller = register_new_seller(self.another_user_id, self.password)
...
######## 添加的测试，测试用户存在，店铺也存在，但这家店铺不是这个用户的
    def test_error_non_exist_user_store_relation(self):
        for b in self.books:
            book_id = b.id
            code = self.another_seller.add_stock_level(self.another_user_id, self.store_id, book_id, 10)
            assert code != 200
