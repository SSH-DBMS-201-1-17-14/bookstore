...
class TestGlobal_search_book_intro:
    ...
    def pre_run_initialization(self):
        ...
        # 注册一个用户进行搜索
        self.user_id = "test_global_search_book_intro_store_id_1_{}".format(time.time())
        ...
        self.another_user_id = "test_global_search_book_intro_store_id_2_{}".format(time.time())
        self.seller = register_new_seller(self.another_user_id, self.password)
        self.store_id = "test_global_search_title_book_intro_id_{}".format(str(uuid.uuid1()))
    ...
    def test_error_page_too_large(self):
        code = self.auth.global_search_book_intro(self.user_id, self.search_info, self.large_page)
        assert code == 532
