...
class TestGlobal_search_title:
    ...
    def test_error_non_user_id(self):
        ...
        code, token = self.auth.global_search_title(user_id, self.search_info, self.page)
        assert code == 511

    #测试用户输入页码太大
    def test_error_page_too_large(self):
        large_page = 20
        code, token = self.auth.global_search_title(self.user_id, self.search_info, large_page)
        assert code == 532
